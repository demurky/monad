...
import (
	...
	"github.com/demurky/monad/moreio"
)
...
func BenchmarkAdapterWriter_WriteString_Adapt_ThreadSafe(b *testing.B) {
	d := discard{}
	a := moreio.NewAdapterWriter(d, moreio.WithThreadSafe())
	b.ResetTimer()
	for b.Loop() {
		a.WriteString("x")
	}
...
func BenchmarkAdapterWriter_WriteByte_Adapt_ThreadSafe(b *testing.B) {
	d := discard{}
	a := moreio.NewAdapterWriter(d, moreio.WithThreadSafe())
	b.ResetTimer()
	for b.Loop() {
...
func BenchmarkAdapterWriter_WriteRune_Adapt_ThreadSafe(b *testing.B) {
	d := discard{}
	a := moreio.NewAdapterWriter(d, moreio.WithThreadSafe())
	b.ResetTimer()
	for b.Loop() {
